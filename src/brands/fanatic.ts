...
class Fanatic extends Scraper<VariantType> {
  ...
  protected async getBrandInfo(): Promise<Brand> {
    ...
    const infoUrl = 'https://www.fanatic.com/windsurf/our-world/about-us/the-brand';

    const extract1 = await this.crawler.crawl(infoUrl, () => {

      const logoLink = (document.querySelector(".logo-small") as HTMLDivElement);

      const logo = window.getComputedStyle(logoLink, null).backgroundImage.slice(4, -1).replace(/"/g, "");

      const motto = (document.querySelector('.page-product-wrap > h2:nth-child(2)') as HTMLHeadingElement).innerText;

      const description =
          (document.querySelector(".text-center") as HTMLDivElement).innerText
          + (document.querySelector("html body div.page-wrap div.page-content section.page.frame-default div.container div.two-text-column-banner") as HTMLDivElement).innerText;

      const picture = (document.querySelector(".full-page-banner > img:nth-child(1)") as HTMLImageElement).src;

      const links = Array.from(document.querySelectorAll(".footer__social-buttons > a"))
