...
class JPAustralia extends FileWriter<VariantType> {
    ...
    async getBrandInfo(): Promise<Brand> {
        ...
        const infoUrl =
            "https://jp-australia.com/who-we-are/";
        ...
        const extract1 = await this.crawler.crawl(infoUrl, () => {
            ...
            const pictures = Array.from(document.querySelectorAll<HTMLImageElement>(".vc_single_image-img"))
                .map(i => i.src)

            const links = Array.from<HTMLAnchorElement>(document.querySelectorAll(".footer-social-media-links a"))
                .map(a => a.href)
                .filter(unique);
