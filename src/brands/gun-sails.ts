...
class GunSails extends FileWriter<VariantType> {
    ...
    async getBrandInfo(): Promise<Brand> {
        ...
        const infoUrl =
            "https://gunsails.com/en/why-gunsails";
        ...
        const extract1 = await this.crawler.crawl(infoUrl, () => {
            ...
            const description =
                Array.from(document.querySelectorAll(".widget-html p"))
                    .map(p => p.firstChild)
                    .filter(e => e !== null && e?.nodeName !== 'A')
                    .map((e: HTMLParagraphElement) => e.textContent)
                    .join('\n\n')

            const pictures = Array.from(document.querySelectorAll<HTMLImageElement>(".col-lg-12 img"))
                .map(i => i.src)

            const links = Array.from<HTMLAnchorElement>(document.querySelectorAll(".social-links a"))
                .map(a => a.href)
                .filter(unique);
