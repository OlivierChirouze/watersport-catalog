...
class Starboard extends FileWriter<VariantType> {
  ...
  async getBrandInfo(): Promise<Brand> {
    ...
    const infoUrl = "https://windsurf.star-board.com/";
    ...
    const extract1 = await this.crawler.crawl(infoUrl, () => {
      ...
      const logo = document.querySelector<HTMLImageElement>(".same-logo img")
        .src;
      ...
      const pictures = Array.from(
        document.querySelectorAll<HTMLImageElement>("rs-slide img")
      ).map(i => i.src);

      const links = Array.from<HTMLAnchorElement>(
        document.querySelectorAll(".soc-ico a")
      )
        .map(a => a.href)
        .filter(unique);
