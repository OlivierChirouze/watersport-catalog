...
import {split, stringToNumber, stringToNumberArray, stringToNumberArrayFiltered} from "../utils";
...
class Neilpryde extends FileWriter<VariantType> {
    ...
    extract(): Extract {
        ...
        const modelName = document.querySelector<HTMLHeadingElement>("h1").innerText

        // Note: need to keep all code inside this method to work in the browser
        const description = document.querySelector<HTMLDivElement>(".ProductMeta__Description > div").innerText
        const table = document.querySelector<HTMLTableElement>("table.tablepress");

        if (table === null || table === undefined)
            throw "Table not found"

        const headers = Array.from(table.querySelectorAll<HTMLTableDataCellElement>('th'))
            .map(h => h.innerText)

        const variantsDiv = Array.from(document.querySelectorAll<HTMLDivElement>(".ProductForm__Option--labelled"))
            .find(div => div.innerText.startsWith("Color"))

        const variants = (variantsDiv === null || variantsDiv === undefined) ? []
        ...
        const pictures = Array.from(document.querySelectorAll<HTMLImageElement>(".flickity-viewport img"))
        ...
        return {data, pictures, variants, description, dimensions, weightVariants};
    }
    ...
    async parse(url: string, modelName: string): Promise<Parsed<VariantType>> {
        ...
        variants = extracted.data.reduce(
            (accumulator: WindsurfSail<VariantType>[], variantData: { [key: string]: string }) => {
                const surfaceM2 = stringToNumber(variantData["surfaceM2"]);
                const weights = split(variantData["weightKg"] ?? '');
                for (let i = 0; i < weights.length; i++) {
                    const construction = extracted.weightVariants[i] === '' ? undefined : extracted.weightVariants[i]

                    accumulator.push({
                        surfaceM2,
                        luffLengthCm: stringToNumber(variantData["luffLengthCm"]),
                        boomLengthsCm: stringToNumberArray(variantData["boomLengthsCm"]),
                        mastExtensionLengthsCm: stringToNumberArray(variantData["mastExtensionLengthsCm"]),
    ...
    async getBrandInfo(): Promise<Brand> {
        ...
        const extract1 = await this.crawler.crawl(infoUrl, () => {
            ...
            const motto = (document.querySelector(".shogun-heading-component") as HTMLDivElement).innerText;
            ...
            const pictures = [
                ...
                ...Array.from(document.querySelectorAll("div[data-bgset]"))
                    .map((div: HTMLDivElement) => div.getAttribute("data-bgset"))
            ]

            const description = Array.from(document.querySelectorAll<HTMLDivElement>("div.shg-theme-text-content"))
    ...
    async parseCatalog(catalog: { year: number, url: string }, productInfo: ProductInfo) {
        const extractedLinks: { name: string, url: string }[] = (await this.crawler.crawl(catalog.url, () => {
            const links = Array.from(document.querySelectorAll<HTMLLinkElement>(".ProductItem__Title a"))

            return links.map(l => ({name: l.innerText, url: l.href}))
        })).map(({name, url}) => ({name: name.replace(catalog.year.toString(), '').trim(), url}))
    ...
    static toProductInfo(productInfo: { [name: string]: Program[] }): ProductInfo {
        return Object.keys(productInfo).reduce(
            (accumulator: ProductInfo, current: string) => {
                accumulator[current.toUpperCase()] = {name: current, programs: productInfo[current]}
                return accumulator;
            },
            {}
        )
    ...
    async parseCatalogs(catalogs: { year: number, url: string }[], productInfo: ProductInfo) {
