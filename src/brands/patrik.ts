...
class Patrik extends Scraper<VariantType> {
    ...
    protected async getBrandInfo(): Promise<Brand> {
        ...
        const extract1 = await this.crawler.crawl(infoUrl, () => {
            ...
            const logo = (document.querySelector("span.logo > a > img") as HTMLImageElement).src;
            const description = (document.querySelector("div.av_one_half:nth-child(1) > section:nth-child(1) > div:nth-child(1) > h3:nth-child(1) > span:nth-child(1)") as HTMLSpanElement).innerText;
            const moto = (document.querySelector("div.av_one_half:nth-child(1) > section:nth-child(1) > div:nth-child(1) > h2:nth-child(2) > span:nth-child(1) > strong:nth-child(1)") as HTMLSpanElement).innerText
                .replace("“", "")
                .replace("”", "");
            const picture = (document.querySelector(".avia-builder-el-4 > div:nth-child(1) > img:nth-child(1)") as HTMLImageElement).src;

            return {logo, description, motto: moto, pictures: [picture]}
        });

        const links = await this.crawler.crawl('https://patrik-windsurf.com/follow-patrik/', () => {
