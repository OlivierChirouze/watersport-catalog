...
export const getText = (text: MultiLanguageText, expectedLanguage: Language) => {
  const language = getBestLanguage(text, expectedLanguage);
  if (!language) return undefined;
  return text[language];
};

export const getBestLanguage = (text: MultiLanguageText, expectedLanguage: Language): Language | undefined => {
